...
class sim_CuriosityEnv3(sim_abs):

    def calcInitPos(self, initPos=None):
        if initPos is None:
            
            x = random.uniform(0.25, 9.0-0.25)
            y = random.uniform(0.25, 4.0-0.25)
    ...
    def loadObstacle(self):
        self.bodyUniqueIds += [self.phisicsClient.loadURDF("urdf/CuriosityEnv3/walls.urdf", basePosition=(0, 0, 0))]
    ...
    def getWallPoints(self):
        return  (np.array([
            ...
            [ 465, -215],
            [-465, -215],
        ]) + np.array([ 450, 200]))/100.0
        # ]) + np.array([ 465, 215]))/100.0
...
class CuriosityEnv3(gym.Env):
    ...
    def __init__(self):
        ...
        self.name = "CuriosityEnv3"
        self.center = (4.3, 3.8)

    def setting(self, _id=-1, mode=p.DIRECT, sec=0.1):
        if _id == -1:
            self.sim = sim_CuriosityEnv3(CuriosityEnv3.global_id, mode, sec)
            CuriosityEnv3.global_id += 1
        else:
            self.sim = sim_CuriosityEnv3(_id, mode, sec)
    ...
    def copy(self, _id=-1):
        new_env = CuriosityEnv3()
        
        if _id == -1:
            new_env.sim = self.sim.copy(CuriosityEnv3.global_id)
            CuriosityEnv3.global_id += 1
        else:
    ...
    def test_reset(self):
        ...
        r = self.reset(initpos=[8.0, 3.0, -math.pi], tgtpos=[8.0, 3.0, -math.pi])
    ...
    def loadWorldMap(self):
        file_name = 'gridmap_CuriosityEnv3.npy'
    ...
    def observe_all(self):
        ...
        return np.concatenate([state, self.sim.getVelocity(), self.sim.getObserve(self.lidar), [1.0-self.completeRate], [left_steps]])
        ...
        # return np.concatenate([state, self.sim.getVelocity(), self.sim.getObserve(self.lidar), [len(self.noveltyBuffer.buffer)],  [(self.world_map_space_num - self.occupancy_map_space_num)*self.gridScan.resolution*self.gridScan.resolution], [left_steps]])
        # return np.concatenate([state, self.sim.getVelocity(), self.sim.getObserve(self.lidar), self.noveltyBuffer.buffer[-1], [len(self.noveltyBuffer.buffer)],  [(self.world_map_space_num - self.occupancy_map_space_num)*self.gridScan.resolution*self.gridScan.resolution], [left_steps]])
